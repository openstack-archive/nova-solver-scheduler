...
scheduler_driver_opts = [
    ...
    cfg.StrOpt('nova_scheduler_admin_user',
               default=None,
               help='Keystone nova-scheduler account username'),
    cfg.StrOpt('nova_scheduler_admin_password',
               default=None,
               help='Keystone nova-scheduler account password'),
    cfg.StrOpt('nova_scheduler_admin_tenant_name',
               default='service',
               help='Keystone nova-scheduler account tenant name'),
    cfg.StrOpt('nova_scheduler_auth_uri',
               default=None,
...
def get_scheduler_context(self):
    username = CONF.nova_scheduler_admin_user
    password = CONF.nova_scheduler_admin_password
    tenant_name = CONF.nova_scheduler_admin_tenant_name
    auth_url = CONF.nova_scheduler_auth_uri
    scheduler_client = keystone_client.Client(username=username, password=password,
                                        tenant_name=tenant_name, auth_url=auth_url)
    scheduler_authref = scheduler_client.auth_ref
    user_id = scheduler_authref.get('user', {}).get('id', None)
    project_id = scheduler_authref['token'].get('tenant', {}).get('id', None)
    auth_token = scheduler_authref['token'].get('id', None)
    service_catalog = scheduler_authref.get('serviceCatalog', None)
    admin_context = novacontext.RequestContext(user_id, project_id,
                        is_admin=True, read_deleted='no', auth_token=auth_token,
