...
class SoftAffinityConstraint(linearconstraints.BaseLinearConstraint):
    """A constraint to ensure that the VMs requested are placed close to each other
        in minimum number of hosts possible."""
    ...
    def update_prob_variable(self, variables, hosts, instance_uuids, request_spec,
        ...
        prob+= pulp.lpSum([column_sum_var[i]] for i in range(self.num_hosts))
        return prob